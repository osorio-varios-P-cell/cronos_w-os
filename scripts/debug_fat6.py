...
# Try mkdir without leading slash
vol.mkdir("EFI")
vol.mkdir("EFI/BOOT")
print("Root after mkdirs:", vol.listdir())

# Create files without leading slash
h = vol.create("limine.cfg")
if h and h.IsValid != False:
    h.write(b"test config")
    h.close()
    print("Created limine.cfg")

h = vol.create("limine-bios.sys")
if h and h.IsValid != False:
    h.write(b"fake limine-bios.sys")
    h.close()
    print("Created limine-bios.sys")

h = vol.create("CRONOS OS")
if h and h.IsValid != False:
    h.write(b"X" * 1000)
    h.close()
    print("Created CRONOS OS")

# Try to create in subdir - maybe we need to open the dir first
# Check if there's a way to change directory
print("Vol path:", vol.path)

# Try using the handle from open
efi_h = vol.open("EFI")
print(f"EFI handle: {efi_h}, IsValid: {efi_h.IsValid if efi_h else None}")
print(f"EFI handle methods: {[m for m in dir(efi_h) if not m.startswith('_')]}")

# Check if Handle has mkdir
if efi_h:
    try:
        efi_h.mkdir("BOOT")
        print("Created EFI/BOOT via handle")
    except Exception as e:
        print(f"mkdir via handle failed: {e}")

    try:
        boot_h = efi_h.create("BOOTX64.EFI")
        print(f"Created BOOTX64.EFI via handle: {boot_h}")
        if boot_h and boot_h.IsValid != False:
            boot_h.write(b"fake BOOTX64.EFI")
            boot_h.close()
    except Exception as e:
        print(f"create via handle failed: {e}")

print("Final root:", vol.listdir())