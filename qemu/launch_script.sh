#!/bin/bash
# Script de lanzamiento para CRONOS W-OS en QEMU
...
CPU_TYPE="host"
CPU_FEATURES="+vmx,+aes,+rdrand,+sse3,+sse4.1,+sse4.2,+popcnt"
CPU_CORES=4
MEMORY_SIZE="16G"
...
# Función de ayuda
show_help() {
    echo "🚀 Script de lanzamiento para CRONOS W-OS"
    echo ""
    echo "Uso: $0 [OPCIONES]"
    echo ""
    echo "Opciones:"
    echo "  -h, --help          Muestra esta ayuda"
    echo "  -d, --debug         Activa modo debug"
    echo "  -g, --gdb           Activa GDB server"
    echo "  -t, --test          Modo testing (auto-exit)"
    echo "  -m, --memory SIZE   Tamaño de memoria (default: 16G)"
    echo "  -c, --cores NUM     Número de cores (default: 4)"
    echo "  -r, --resolution    Resolución de pantalla (default: 1920x1080)"
    echo "  --no-gpu            Desactiva GPU virtual"
    echo "  --no-network        Desactiva red virtual"
    echo "  --no-usb            Desactiva USB virtual"
    echo ""
    echo "Ejemplos:"
    echo "  $0                  Lanza CRONOS W-OS normal"
    echo "  $0 -d -g            Lanza con debug y GDB"
    echo "  $0 -t               Lanza en modo test"
    echo "  $0 -m 8G -c 2       Lanza con 8GB RAM y 2 cores"
}
...
if [ "$NO_GPU" = false ]; then
    QEMU_CMD="$QEMU_CMD -device virtio-gpu-pci,xres=${GPU_RESOLUTION%,*},yres=${GPU_RESOLUTION#*,}"
fi
...
if [ "$DEBUG_MODE" = true ]; then
    QEMU_CMD="$QEMU_CMD -d int,cpu_reset,exec"
fi
